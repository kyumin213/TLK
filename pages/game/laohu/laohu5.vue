<template>
	<view class="lion">
		...
		<view class="bottom">
				<image src="~@/static/game/heyue.png" @tap="seeTequan" class="heyue" mode="widthFix"></image>
			<image src="~@/static/game/yaoqing.png" @tap="seeYaoqing" mode="widthFix"></image>
			<image src="~@/static/game/youxi.png" @tap="seeYouxi" mode="widthFix"></image>
			<image src="~@/static/game/shezhi.png" @tap="seeSetting" mode="widthFix"></image>
		</view>
		...
		<!--合约规则-->
		<view class="heyue_guize" v-show="guizeIf">
			<view class="item">
				<view class="msg">
					<text>
						{{languageInfo.contractCopy}}
					</text>
					<!-- <text v-for="(item,index) in contract" :key='index'>{{item.tlk_num}}</text> -->
					<view>2.</view>
					<view v-for="(item,index) in contract" :key='index'>
						{{'v'+(index+1) + '('+languageInfo.lock + item.tlk_num+'TLK,'+languageInfo.contractPeriod + item.day_num + languageInfo.day+','+languageInfo.everyday +' '+item.lun_num + languageInfo.contractNum}}
					</view>
				</view>
				<view class="back" @tap="back">
					<image src="~@/static/game/back.png" mode="widthFix"></image>
				</view>
				<view class="x" @tap="closeGuize">
					<image src="~@/static/game/x.png" mode="widthFix"></image>
				</view>
			</view>
		</view>
		...
		<view class="share" v-show="yaoqingIf">
			<view class="item">
				...
				<view class="msg" v-show="bindId === 0">
					...
					<!-- <text v-if="shareObj.is_bind===1">已绑定</text> -->
					<!-- <text v-if="shareObj.is_bind===2">未绑定</text> -->
				...
				<!-- <view class="tuiguang" @tap="seeHistory"><text>{{languageInfo.test15}}>></text></view> -->
			</view>
		</view>
		...
		<view class="youxi" v-show="youxiIf">
			<view class="item">
				...
				<!-- <view class="list" v-if="historyList.length===0">
					<text>{{languageInfo.noRecord}}</text>
				</view> -->
...
<script>
	...
	export default {
		...
		mounted() {
			...
			this.get('api/myGameLog',{rowNum:15}).then((res)=>{
				console.log(res)
			...
			this.get('api/personExpandLog',{expand_type:1}).then((res)=>{
				// console.log(res)
		...
		methods: {
			...
			suoding(){
				...
				this.post('api/buyContract',{contract_id:this.suocangId}).then((res)=>{
					console.log(res)
				})//购买合约
			},
			closeSuoCang(){
			...
			bangding(){
				this.get('api/bindUser',{invite_code:this.shareObj.inviteCode}).then((res)=>{
	...
	.lion{
		...
		.child{
			...
			.suocang{
				...
				.item{
					width: 1000rpx;
					background: url(../../../static/game/suocang.png) no-repeat;
					background-size: 100% 85%;
					position: relative;
					display: flex;
					flex-direction: column;
